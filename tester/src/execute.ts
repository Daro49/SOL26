...
function runProcess(bin: string, args: string[], logger: Logger): ProcessResult {
  logger.debug("Running: python %s %s", bin, args.join(" "));

  const result = spawnSync("python", [bin, ...args], {
    encoding: "utf8",
...
function diffStrings(expected: string, actual: string): string | null {
  ...
  try {
    const result = spawnSync("diff", ["-Z", expectedFile, actualFile], { encoding: "utf8" });
...
function executeParseOnly(
  ...
  try {
    const r = runProcess(config.parserBin, [sourceFile], config.logger);

    if (r.failed)
      return new UnexecutedReason(
        UnexecutedReasonCode.CANNOT_EXECUTE,
        `Parser could not be executed: ${r.stderr}`
      );
...
function executeInterpretOnly(
  ...
  try {
    ...
    if (r.failed)
      return new UnexecutedReason(
        UnexecutedReasonCode.CANNOT_EXECUTE,
        `Interpreter could not be executed: ${r.stderr}`
      );
...
export function executeTestCase(
  ...
  switch (tc.definition.test_type) {
    case TestCaseType.PARSE_ONLY:
      return executeParseOnly(tc, config);

    case TestCaseType.EXECUTE_ONLY:
      return executeInterpretOnly(tc, config);

    case TestCaseType.COMBINED:
      return executeCombined(tc, config);
  }
}
